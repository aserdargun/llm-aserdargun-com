...
export function ExplorePage() {
  ...
  const rail = <div className="filter-content">
    ...
    {filterKeys.map((key) => <details key={key} open={['category', 'hardware'].includes(key)}><summary>{labels[key]}{state[key].length > 0 && <span>{state[key].length}</span>}</summary><div className="filter-options">{valuesFor[key].map((value) => <label key={value}><input type="checkbox" checked={state[key].includes(value)} onChange={() => toggle(key, value)} /><span>{value}</span></label>)}</div></details>)}
