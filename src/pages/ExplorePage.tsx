...
export function ExplorePage() {
  ...
  useEffect(() => {
    if (drawer) filterDrawer.current?.focus()
  }, [drawer])
  const closeDrawer = () => {
    setDrawer(false)
  ...
  return <div className="shell page-shell explore-page">
    ...
    {drawer && <div className="drawer-backdrop"><button className="drawer-scrim" type="button" aria-label={pick(locale, 'Filtreleri kapat', 'Close filters')} onClick={closeDrawer} /><aside ref={filterDrawer} className="filter-drawer" role="dialog" aria-modal="true" aria-label={pick(locale, 'Filtreler', 'Filters')} tabIndex={-1} onKeyDown={(event) => { if (event.key === 'Escape') closeDrawer() }}><div className="drawer-head"><h2>{pick(locale, 'Filtreler', 'Filters')}</h2><button type="button" aria-label={pick(locale, 'Filtreleri kapat', 'Close filters')} onClick={closeDrawer}><X /></button></div>{rail}<button className="button primary drawer-apply" type="button" onClick={closeDrawer}>{results.length} {pick(locale, 'sonucu göster', 'show results')}</button></aside></div>}
