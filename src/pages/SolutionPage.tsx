...
import { displayAtlasValues, sourceTypeLabel } from '@/i18n/atlas-labels'
...
export function SolutionPage() {
  ...
  return <article className="shell page-shell detail-page">
    ...
    {solution.projectStatus === 'archived' && <div className="layer-warning" role="alert">{pick(locale, 'Bu proje arşivlendi ve kaynak deposu salt okunur durumda. Kayıt yalnızca tarihsel ekosistem bağlamı için korunuyor.', 'This project is archived and its source repository is read-only. The record is retained only for historical ecosystem context.')}</div>}
    ...
    <section className="compatibility"><div className="section-heading"><Layers3 /><h2>{pick(locale, 'Uyumluluk görünümü', 'Compatibility view')}</h2></div><dl><div><dt><Term tip={fieldTip('hardware', locale)}>{pick(locale, 'Donanım', 'Hardware')}</Term></dt><dd>{displayAtlasValues(locale, solution.hardware)}</dd></div><div><dt><Term tip={fieldTip('backend', locale)}>{pick(locale, 'Arka uçlar', 'Backends')}</Term></dt><dd>{displayAtlasValues(locale, solution.executionBackends)}</dd></div><div><dt><Term tip={fieldTip('format', locale)}>{pick(locale, 'Model formatları', 'Model formats')}</Term></dt><dd>{displayAtlasValues(locale, solution.modelFormats)}</dd></div><div><dt><Term tip={fieldTip('protocol', locale)}>{pick(locale, 'API protokolleri', 'API protocols')}</Term></dt><dd>{displayAtlasValues(locale, solution.apiProtocols)}</dd></div><div><dt><Term tip={fieldTip('scope', locale)}>{pick(locale, 'Dağıtım', 'Deployment')}</Term></dt><dd>{displayAtlasValues(locale, solution.deploymentScopes)}</dd></div><div><dt><Term tip={fieldTip('license', locale)}>{pick(locale, 'Lisans', 'License')}</Term></dt><dd>{solution.license}</dd></div></dl></section>
    ...
    <section className="sources"><h2>{pick(locale, 'Resmî kaynaklar', 'Official sources')}</h2>{solution.sources.map((source) => <ExternalLink key={source.url} href={source.url}><span><strong>{source.title}</strong><small>{source.publisher} · {sourceTypeLabel(locale, source.sourceType)} · {source.verifiedAt}</small></span></ExternalLink>)}</section>
  </article>
}
