...
import { guideConflicts, matchGuide, type GuideAnswers } from '@/features/guide/matcher'
...
export function GuidePage() {
  ...
  const conflicts = complete ? guideConflicts(answers as GuideAnswers) : []
  ...
  return <div className="shell page-shell guide-page">
    ...
    {!complete ? <section className="question-card"><div className="progress-line"><span className="mono">{pick(locale, `Soru ${step + 1} / 5`, `Question ${step + 1} of 5`)}</span><div><i style={{ width: `${((step + 1) / 5) * 100}%` }} /></div></div><h2>{questions[step]![locale]}</h2><div className="answer-grid">{questions[step]!.options.map((option) => <button key={option.value} type="button" onClick={() => choose(questions[step]!.key, option.value)}>{option[locale]}<ArrowRight size={18} /></button>)}</div>{step > 0 && <button className="text-button back-question" type="button" onClick={() => setStep((current) => current - 1)}><ArrowLeft size={16} />{pick(locale, 'Önceki soru', 'Previous question')}</button>}</section> : <section className="guide-results"><div className="results-title"><span className="mono">{pick(locale, 'GEREKÇELİ KISA LİSTE', 'JUSTIFIED SHORTLIST')}</span><h2>{pick(locale, 'Senaryonuza uyan yollar', 'Paths that fit your scenario')}</h2><button className="button secondary" type="button" onClick={restart}><RotateCcw size={17} />{pick(locale, 'Yeniden başla', 'Start over')}</button></div>{matches.length === 0 && <div className="notice" role="status"><h3>{pick(locale, 'Bu seçimlerle doğrulanmış bir yol bulunamadı', 'No verified path for these selections')}</h3>{conflicts.length ? conflicts.map((conflict, index) => <p key={index}>{conflict[locale]}</p>) : <p>{pick(locale, 'Donanım ve platform tercihlerini yeniden değerlendirin. Boş liste, tüm ekosistemde çözüm olmadığı anlamına gelmez.', 'Reconsider the hardware and platform choices. An empty list does not mean no solution exists in the wider ecosystem.')}</p>}</div>}<div className="recommendations">{matches.map((match, index) => { const solution = solutions.find((item) => item.slug === match.slug)!; return <article key={match.slug}><span className="mono">0{index + 1} · {solution.primaryCategory}</span><h3>{solution.name}</h3><p>{match.reason[locale]}</p><small><strong>{pick(locale, 'Dikkat:', 'Caveat:')}</strong> {match.caveat[locale]}</small><div><Link className="text-link" to={`/${locale}/solutions/${solution.slug}`}>{pick(locale, 'Profili aç', 'Open profile')}<ArrowRight size={16} /></Link><Link className="text-link" to={`/${locale}/compare?compare=${matches.map(({ slug }) => slug).join(',')}`}>{pick(locale, 'Kısa listeyi karşılaştır', 'Compare shortlist')}</Link></div></article> })}</div></section>}
  </div>
}
