  ...
  return <>
    <section className="home-hero">
      ...
      <div className="shell hero-grid"><div className="hero-copy">
        ...
        <div><span className="mono">31</span><small>{pick(locale, 'kaynaklı çözüm profili', 'source-backed solution profiles')}</small></div>
      ...
      <div className="shell hero-source mono"><span><CircleDot size={12} />{pick(locale, 'RESMÎ BELGE VE DEPO KAYNAKLARI', 'OFFICIAL DOCUMENTATION AND REPOSITORIES')}</span><span>{pick(locale, 'KAYNAK ERİŞİM DENETİMİ', 'SOURCE ENDPOINT REVIEW')} · {ATLAS_DATASET_REVIEWED_AT}</span></div>
    </section>
    ...
    <section className="shell evidence-section"><div className="section-intro"><span className="mono">03 · {pick(locale, 'KARŞILAŞTIRMA VE KANIT', 'COMPARISON AND EVIDENCE')}</span><div><h2>{pick(locale, 'Aynı soruyu, doğru katmanda sorun.', 'Ask the same question in the right layer.')}</h2><p>{pick(locale, 'Her profil rol, donanım, dağıtım kapsamı ve resmî kaynak iziyle birlikte okunur.', 'Every profile is read with its role, hardware, deployment scope, and official-source trail.')}</p></div></div><div className="evidence-table-wrap"><table className="evidence-table"><thead><tr><th>{pick(locale, 'ÇÖZÜM', 'SOLUTION')}</th><th>{pick(locale, 'KATMAN', 'LAYER')}</th><th>{pick(locale, 'DONANIM', 'HARDWARE')}</th><th>{pick(locale, 'DAĞITIM', 'DEPLOYMENT')}</th><th>{pick(locale, 'KANIT', 'EVIDENCE')}</th></tr></thead><tbody>{featured.map((solution) => { const category = categories.find((item) => item.id === solution.primaryCategory)!; return <tr key={solution.slug}><th><Link to={`/${locale}/solutions/${solution.slug}`}>{solution.name}<ArrowRight size={15} /></Link></th><td><span className="mono">{solution.primaryCategory}</span> {category.name[locale]}</td><td>{displayAtlasValues(locale, solution.hardware.slice(0, 2))}</td><td>{displayAtlasValues(locale, solution.deploymentScopes.slice(0, 3))}</td><td><span className="evidence-state"><CircleDot size={11} />{pick(locale, 'Resmî kaynak', 'Official source')}</span></td></tr> })}</tbody></table></div><div className="evidence-actions"><Link className="button secondary" to={`/${locale}/explore`}>{pick(locale, '31 kaydın tamamını keşfet', 'Explore all 31 records')}<ArrowRight size={17} /></Link><Link className="text-link" to={`/${locale}/methodology`}>{pick(locale, 'Metodolojiyi incele', 'Read the methodology')}</Link></div></section>
    <div className="shell"><PortfolioLearning /></div>
  </>
}
