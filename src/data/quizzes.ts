...
export const quizzes: QuizQuestion[] = [
  ...
  },
  {
    ...
    correct: 1,
    explain: {
      tr: 'KV cache, her katman için (num_heads, seq_len, head_dim) tensörünü saklar; önceki hesabı yeniden yapmadan decode hızını 5-10x artırır. Uzun bağlamda GPU belleğinin en büyük tüketicisidir.',
      en: 'KV cache stores the K/V matrices per layer; it avoids recomputing attention at each step, boosting decode speed 5-10x. It is the biggest consumer of GPU memory in long-context scenarios.',
    },
  ...
  },
  {
    ...
    correct: 1,
    explain: {
      tr: 'Continuous batching, her decoding iterasyonunda biten istekleri gruptan çıkarır; kısa istekler uzun olanları beklemediği için verim 2-4x artar. vLLM bu yaklaşımı varsayılan yapan ilk motordur.',
      en: 'Continuous batching removes finished requests at each decoding iteration; short requests no longer wait for long ones, lifting throughput 2-4x. vLLM was the first engine to make this the default.',
    },
  ...
  },
  {
    ...
    correct: 2,
    explain: {
      tr: 'PagedAttention, işletim sistemlerindeki "virtual memory + page table" modelini ödünç alarak bellek parçalanmasını ortadan kaldırır ve kullanımı neredeyse %100\'e çıkarır.',
      en: 'PagedAttention borrows the "virtual memory + page table" model from operating systems; it eliminates fragmentation and pushes memory utilization close to 100%.',
    },
  ...
  },
  {
    ...
    correct: 0,
    explain: {
      tr: 'Prefill compute-bound, decode memory-bandwidth-bound çalışır. Aynı GPU\'da birbirlerini yavaşlatırlar; ayrı donanımlara bölmek 1.5-2x verim kazancı sağlar.',
      en: 'Prefill runs compute-bound, decode runs memory-bandwidth-bound. On the same GPU they slow each other down; splitting them onto separate hardware gives 1.5-2x throughput gain.',
    },
  ...
  },
  {
    ...
    options: [
      { tr: 'Belleği %75 azaltır, hız artar; kalite ihmal edilebilir düzeyde düşer', en: 'Cuts memory by 75%, speeds up inference; quality drops negligibly' },
    ...
    correct: 0,
    explain: {
      tr: '4 bit / 16 bit = %75 bellek tasarrufu ve hız kazancı. AWQ ve GPTQ gibi iyi uygulanmış yöntemler kalite kaybını büyük ölçüde azaltır; çoğu kullanım senaryosunda ihmal edilebilir fark vardır.',
      en: '4 bits vs 16 bits = 75% memory savings and speedup. Well-applied methods like AWQ and GPTQ minimize quality loss; the difference is negligible for most use cases.',
    },
  ...
  },
  {
    ...
    correct: 1,
    explain: {
      tr: 'LoRA, ΔW = A·B (r≪d) şeklinde düşük ranklı adaptörler ekler. Llama 7B için %0.1-0.5 eğitilebilir parametre, 3-4x bellek tasarrufu ve 10x eğitim hızı kazancı sağlar.',
      en: 'LoRA adds low-rank adapters as ΔW = A·B with r≪d. For Llama 7B that is 0.1-0.5% trainable parameters, 3-4x memory savings, and 10x training speedup.',
    },
  ...
  },
  {
    ...
    kind: 'mcq',
    prompt: {
      tr: 'Tüketici sınıfı NVIDIA GPU\'larda EXL2/EXL3 düşük bitli formatlarla nicemlenmiş çıkarıma odaklanan deneysel motor hangisidir?',
      en: 'Which experimental engine focuses on low-bit (EXL2/EXL3) quantized inference on consumer NVIDIA GPUs?',
    },
    options: [
      { tr: 'ExLlamaV3', en: 'ExLlamaV3' },
      { tr: 'llama.cpp', en: 'llama.cpp' },
    ...
    correct: 0,
    explain: {
      tr: 'ExLlamaV3, tek kullanıcı/yerel GPU senaryolarında düşük bitli nicemlenmiş modeller için optimize edilmiştir. Hızlı değişen deneysel bir projedir.',
      en: 'ExLlamaV3 is optimized for low-bit quantized models on single-user / local GPUs. It is a fast-moving experimental project.',
    },
  ...
  },
  {
    ...
    correct: 0,
    explain: {
      tr: 'NVIDIA Dynamo, çok düğümlü yüksek ölçekli GPU kümeleri için disaggregated serving mimarisi sunar; TensorRT-LLM, vLLM ve SGLang motorlarını koordine eder.',
      en: 'NVIDIA Dynamo provides a disaggregated serving architecture for high-scale multi-node GPU clusters; it coordinates TensorRT-LLM, vLLM, and SGLang engines.',
    },
  ...
  },
  {
    ...
    correct: 0,
    explain: {
      tr: 'WebLLM, model indirme/önbellekleme, worker desteği ve OpenAI benzeri API ile tarayıcıda veri gizliliği sağlar. Veri cihazdan dışarı çıkmaz, GDPR/KVKK uyumu kolaylaşır.',
      en: 'WebLLM enables in-browser inference with model cache, workers, and an OpenAI-like API; data never leaves the device, easing GDPR/KVKK compliance.',
    },
...

  {
    ...
    kind: 'mcq',
    prompt: {
      tr: 'Apple Silicon (M2/M3/M4) Mac\'te 8B modeli denemek istiyorsunuz. Hangi kombinasyon en uygundur?',
      en: 'You want to try an 8B model on an Apple Silicon (M2/M3/M4) Mac. Which combination is most appropriate?',
    },
    options: [
  ...
  },
  {
    ...
    kind: 'mcq',
    prompt: {
      tr: 'Bir agent uygulamasında modelin "sadece şu JSON şemasına uygun cevap ver" demesini istiyorsunuz. Hangi sampling ayarı zorunludur?',
      en: 'In an agent app you want the model to "respond only with this JSON schema". Which sampling setting is mandatory?',
    },
    options: [
      { tr: 'Temperature=1, Top-p=0.9', en: 'Temperature=1, Top-p=0.9' },
      { tr: 'Temperature=0, Top-p=1', en: 'Temperature=0, Top-p=1' },
      { tr: 'Temperature=2, Top-p=0.5', en: 'Temperature=2, Top-p=0.5' },
      { tr: 'Temperature=0.5, Top-p=0.7', en: 'Temperature=0.5, Top-p=0.7' },
    ],
    correct: 1,
    explain: {
      tr: 'Structured output üretirken rastgelelik şemayı bozar; temperature=0 ve top-p=1 ile deterministik sampling şarttır. Aksi halde JSON parse hataları olur.',
      en: 'When producing structured output, randomness breaks the schema; temperature=0 and top-p=1 are required for deterministic sampling. Otherwise JSON parsing fails.',
    },
  ...
  },
  {
    ...
    correct: 1,
    explain: {
      tr: '70B FP16 ~140 GB. INT4 nicemleme ile ~35 GB\'a düşer; AWQ gibi yöntemler kaliteyi korur. 24 GB\'a sığdırmak için ek offloading veya daha agresif nicemleme gerekebilir.',
      en: '70B in FP16 is ~140 GB. INT4 brings it to ~35 GB; AWQ preserves quality. To fit 24 GB you may also need offloading or more aggressive quantization.',
    },
  ...
  },
  {
    ...
    kind: 'mcq',
    prompt: {
      tr: 'Tarayıcıda, kullanıcının verisini hiçbir sunucuya göndermeden çalışan bir yapay zekâ özelliği istiyorsunuz. Hangi çözüm en uygundur?',
      en: 'You want a browser-based AI feature that never sends user data to any server. Which solution is most appropriate?',
    },
    options: [
      { tr: 'Ollama', en: 'Ollama' },
    ...
    correct: 1,
    explain: {
      tr: 'WebLLM, modeli tarayıcıda WebGPU ile çalıştırır; veri cihazdan dışarı çıkmaz, gizlilik üst düzeydedir. Diğerleri sunucu tarafında çalışır.',
      en: 'WebLLM runs the model in the browser via WebGPU; data never leaves the device, so privacy is top tier. The others run server-side.',
    },
  ...
  },
  {
    ...
    correct: 1,
    explain: {
      tr: 'NVIDIA Dynamo, llm-d gibi disaggregated serving çözümleri prefill ve decode\'u farklı GPU\'lara ayırarak 1.5-2x verim kazancı sağlar.',
      en: 'Disaggregated serving solutions like NVIDIA Dynamo and llm-d split prefill and decode onto different GPUs, yielding 1.5-2x throughput gain.',
    },
  ...
  },
  {
    ...
    correct: 1,
    explain: {
      tr: 'LoRA/QLoRA, 70B modeli tek bir 24GB GPU\'da eğitilebilir hale getirir; full FT\'nin kalitesine yakın sonuçlar verir. Prompt engineering ve RAG farklı katmanlarda kalır.',
      en: 'LoRA/QLoRA make a 70B model trainable on a single 24 GB GPU, with quality close to full FT. Prompt engineering and RAG stay on different layers.',
    },
  ...
  },
  {
    ...
    correct: true,
    explain: {
      tr: 'Temperature=0 greedy decoding\'dir; çoğu motor aynı girdi + T=0 + aynı seed ile tekrarlanabilir sonuç verir. Ancak bazı dağıtık/atomic operasyonlarda floating-point sırası farklılık gösterebilir; tam reproducibility için seed de sabitlenmelidir.',
      en: 'Temperature=0 is greedy decoding; most engines return the same output for the same input + T=0 + same seed. Some distributed/atomic operations can still differ in floating-point order; pin the seed too for full reproducibility.',
    },
  ...
  },
  {
    ...
    correct: false,
    explain: {
      tr: 'AWQ veya GPTQ gibi iyi uygulanmış INT4 niceleme çoğu benchmarkta ihmal edilebilir kalite kaybı verir. QAT (Quantization-Aware Training) ile kalite korunabilir. Kalite, sadece düşük bit + dikkatsiz uygulamada düşer.',
      en: 'Well-applied INT4 methods like AWQ or GPTQ give negligible quality loss on most benchmarks. QAT (Quantization-Aware Training) preserves quality. Quality drops only with low bits plus careless application.',
    },
  ...
  },
  {
    ...
    correct: false,
    explain: {
      tr: 'Eğer LoRA adaptörü ana ağırlıklara merge edilmezse, her inference\'ta ek matris çarpımı (A·B) yapılır. vLLM ve llama.cpp merge edilmiş saf modeli tercih eder; merge edilmezse %5-15 ek gecikme olabilir.',
      en: 'If the LoRA adapter is not merged into the main weights, every inference pays for the extra A·B multiplication. vLLM and llama.cpp prefer the merged plain model; unmerged inference can add 5-15% latency.',
    },
  ...
  },
  {
    ...
    correct: 1,
    explain: {
      tr: 'Alan-özgü ince ayar için LoRA/QLoRA pratik standarttır: küçük bir eğitim verisi ve tek bir 24 GB GPU yeterlidir. Full fine-tune pahalıdır ve çoğu senaryoda gerekmez.',
      en: 'LoRA/QLoRA is the practical standard for domain specialization: a small training set and a single 24 GB GPU are enough. Full fine-tuning is expensive and unnecessary for most scenarios.',
    },
  ...
  },
  {
    ...
    correct: 1,
    explain: {
      tr: 'Veri gizliliği şartı varsa on-prem/self-hosted çözüm şarttır. llama.cpp (CPU/Metal) veya Ollama, modeli tamamen kendi altyapınızda çalıştırır; hiçbir veri dışarı çıkmaz.',
      en: 'When data residency is required, on-prem/self-hosted is mandatory. llama.cpp (CPU/Metal) or Ollama runs the model entirely inside your infrastructure; no data leaves.',
    },
  ...
  },
  {
    ...
    correct: false,
    explain: {
      tr: 'Llama, Mistral, Qwen gibi açık kaynak modeller dünya genelinde binlerce üretim sisteminde çalışır. Doğru motor (vLLM, SGLang, TGI) ve izleme ile ticari SLA seviyesinde hizmet verebilir.',
      en: 'Open-source models like Llama, Mistral, and Qwen run in thousands of production systems. With the right engine (vLLM, SGLang, TGI) and observability, they can serve commercial-grade SLAs.',
    },
  ...
  },
  {
    ...
    kind: 'truefalse',
    prompt: {
      tr: 'Apple Silicon (M1/M2/M3/M4) üzerinde LLM çalıştırılamaz.',
      en: 'LLMs cannot run on Apple Silicon (M1/M2/M3/M4).',
    },
    correct: false,
    explain: {
      tr: 'Apple Silicon, unified memory avantajıyla 70B+ modelleri bile çalıştırabilir. mlx-lm (Apple\'ın MLX framework\'ü) ve llama.cpp Metal backend bunu mümkün kılar; bazı benchmarklarda NVIDIA ile yarışır.',
      en: 'Apple Silicon can run 70B+ models thanks to unified memory. mlx-lm (Apple\'s MLX framework) and the llama.cpp Metal backend make this possible; on some benchmarks they rival NVIDIA.',
    },
